...
**Aging Thundering Herd 문제**

P1_BURST 구간처럼 Q3/Q4/Q5에 항목이 대량으로 쌓인 뒤 STARVATION 구간으로 전환되면, 비슷한 시점에 유입된 항목들이 **동시에 aging 임계값(10초)을 초과**하여 Q2로 일괄 방출됩니다. 개별적으로 조금씩 승격되지 않고 한꺼번에 몰리는 현상입니다.

해결하려면 아래 중 하나가 필요하나 시간 관계상 구현하지 못했습니다.

*   **승격 속도 제한**: 1 aging 사이클당 Q2로 올릴 수 있는 항목 수를 N개로 제한
*   **지터(Jitter) 추가**: aging 임계값에 랜덤 오프셋을 더해 동시 만료 방지
*   **확률적 승격**: 대기 시간이 길수록 승격 확률이 높아지는 점진적 방식
